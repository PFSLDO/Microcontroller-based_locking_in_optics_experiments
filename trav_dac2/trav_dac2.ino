...
const int dacPin = 25;   // DAC1 interno
float valueFloat = 0.0;  // agora usamos float
float Kp = 0.02;         // ganho proporcional (ajuste fino depois)
...
void writeDitheredDAC(float v) {
  ...
  dacWrite(dacPin, base);

  // probabilidade proporcional à parte fracionária
  if (random(1000) < frac * 1000.0) {
    dacWrite(dacPin, base + 1);
  }
}
...
void loop() {
  ...
  if (currentSystemMode == SWEEP) {
    ...
    valueFloat += direction * (stepSize / 16.0); 
    if (value >= resolution) {
      value = resolution;
      direction = -1;
    } else if (value <= 0) {
    ...
    int erro = averageAdcValue - lastAdcValue;
  
    valueFloat += Kp * erro;
  
    // Limites do DAC 8 bits
    if (valueFloat < 0) valueFloat = 0;
    if (valueFloat > 255) valueFloat = 255;
